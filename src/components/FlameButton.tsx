...
const FlameButton = forwardRef<HTMLButtonElement, FlameButtonProps>(
  (
    ...
    const variants = {
      primary:
        'bg-hotteach-yellow text-hotteach-dark hover:shadow-lg hover:brightness-105',
      secondary:
        'bg-hotteach-red text-white hover:shadow-lg hover:brightness-105',
      outline:
        'bg-transparent border-2 border-hotteach-yellow text-hotteach-yellow hover:bg-hotteach-yellow/10',
    ...
    return (
      <Component
        className={cn(
          'relative font-medium transition-all duration-300 ease-out flex items-center justify-center gap-2 hover-scale cursor-pointer',
          variants[variant],
          sizes[size],
        ...
        {showFlame && !isLoading && (
          <Flame
            className={cn(
              'w-5 h-5',
              size === 'xl' && 'w-6 h-6',
              'text-hotteach-red animate-flame-pulse'
            )}
          />
        )}
        {isLoading ? (
